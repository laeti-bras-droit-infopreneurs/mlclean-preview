---
title: "Enlèvement d'encombrants"
pole: debarras-vidage
accroche: >-
  Meubles, électroménager, literie, gros volumes : nous enlevons, nous
  trions, et ce qui peut servir est donné plutôt que jeté.
description: >-
  Enlèvement d'encombrants à Sarlat et alentours : meubles, électroménager,
  literie, gros volumes. Tri systématique, don et recyclage privilégiés.
  Devis gratuit sous 72 h.
---

Un canapé à évacuer, un électroménager en panne, une literie à remplacer, ou
tout ce qui reste après des travaux ou un déménagement : quand les objets sont
trop lourds, trop gros ou trop nombreux pour la déchetterie, nous nous en
chargeons, **du logement jusqu'à la bonne filière**.

## Ce que nous enlevons

- **Mobilier** : canapés, armoires, literie, tables, bibliothèques ;
- **électroménager** : réfrigérateurs, machines à laver, cuisinières
  (orientés vers les filières de recyclage adaptées) ;
- **gros volumes** : cartons accumulés, textiles, vaisselle, restes de
  déménagement ou de travaux ;
- **ferraille et matériaux** divers.

Nous intervenons dans les étages, sans ascenseur si nécessaire, et dans les
accès difficiles.

## Jeter le moins possible

C'est notre différence d'entreprise sociale et solidaire : un encombrant
n'est pas forcément un déchet.

- Ce qui est en bon état part en **don ou réemploi** dans les filières
  locales ;
- l'électroménager et la ferraille partent au **recyclage** ;
- seul le reste va en déchetterie.

Vos encombrants peuvent avoir une seconde vie utile à quelqu'un du territoire,
et cela réduit souvent le coût d'évacuation.

## Ponctuel ou dans un projet plus large

L'enlèvement d'encombrants peut être une intervention simple et rapide, ou la
première étape d'un projet plus large :
[débarras complet]({{ '/prestations/debarras-vidage/' | relative_url }}),
[remise en état avant vente]({{ '/situations/remise-en-etat-avant-vente/' | relative_url }}),
[désencombrement de sécurité]({{ '/situations/maintien-a-domicile/' | relative_url }})
chez une personne âgée. Parlez-nous de votre situation : le devis est gratuit
et sous 72 h.

Intervention à Sarlat et alentours, maison, appartement, dépendances.
